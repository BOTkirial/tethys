...
class Terrain {
    ...
    init(scene) {
        ...
        const boxGeometry = new THREE.BoxGeometry(10, 1, 10, 10, 1, 10);
        const boxMaterial = new THREE.MeshBasicMaterial({ color: 0xFF00FF });
        const boxMesh = new THREE.Mesh(boxGeometry, boxMaterial);
        for (let cpt = 0; cpt < boxMesh.geometry.attributes.position.array.length; cpt += 3) {
            let x = boxMesh.geometry.attributes.position.array[cpt + 0];
            let y = boxMesh.geometry.attributes.position.array[cpt + 1];
            let z = boxMesh.geometry.attributes.position.array[cpt + 2];
            console.log(new THREE.Vector3(x, y, z));

            if (y > 0) {
                let newY = 100;
                boxMesh.geometry.attributes.position.array[cpt + 1] = newY;
            }
        }
        boxMesh.position.set(10, 100, 10)
        scene.add(boxMesh)
